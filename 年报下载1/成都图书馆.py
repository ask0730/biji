...
"""
成都图书馆年报下载脚本
功能：从成都图书馆官网下载去年的年报，并重命名为"成都图书馆年份年报"格式
"""
...
def find_last_year_report(url, base_url):
    ...
    try:
        ...
        for link in soup.find_all('a', href=True):
            ...
            has_juesuan = '决算' in check_text or '部门决算' in check_text
            
            # 在服务数据页面，只需要包含"年报"或"年度报告"即可
            # 或者包含"图书馆"和"决算"/"年报"
            is_service_data_page = 'activity.html' in url.lower() or 'dynamic' in url.lower()
            if is_service_data_page:
                # 服务数据页面：只需要包含"年报"或"年度报告"
                if not has_report:
                    continue
            else:
                # 其他页面：必须包含"图书馆"，并且包含"决算"或"年报"
            ...
            if html_links:
                ...
                for link_info in html_links:
                    ...
                    try:
                        ...
                        # 首先在页面源码中查找所有PDF URL（包括JavaScript代码、注释等）
                        print("  正在分析页面源码查找PDF链接...")
                        page_source = link_response.text
                        # 查找所有PDF URL（包括相对路径和绝对路径）
                        pdf_url_patterns = [
                            r'https?://[^\s<>"\'\)]+\.pdf',  # 绝对URL
                            r'/[^\s<>"\'\)]+\.pdf',  # 相对路径
                            r'\.\./[^\s<>"\'\)]+\.pdf',  # 相对路径
                        ...
                        for pattern in pdf_url_patterns:
                            ...
                            for match in matches:
                                ...
                                if match.startswith('http'):
                                    ...
                                    pdf_url = urljoin(html_url, match)
                                if pdf_url.lower().endswith('.pdf') and pdf_url not in found_pdf_urls:
                                    found_pdf_urls.append(pdf_url)
                        
                        # 如果找到PDF URL，优先使用
                        if found_pdf_urls:
                            print(f"  在页面源码中找到 {len(found_pdf_urls)} 个PDF链接")
                            ...
                            # 如果没有匹配年份的，返回第一个
                            print(f"✓ 在页面源码中找到PDF（未验证年份）: {found_pdf_urls[0]}")
                            return found_pdf_urls[0], last_year_str
...
def main():
    ...
    # 成都图书馆年报页面URL（服务数据页面）
    report_page_url = "https://www.cdclib.cn/mapping/dynamic/activity.html"
    base_url = "https://www.cdclib.cn"
    ...
    filename = f"成都图书馆{year_for_filename}年年报{file_ext}"
    filename = clean_filename(filename)
