...
"""
河北省图书馆年报下载脚本
功能：从河北省图书馆官网下载去年的年报，并重命名为"河北省图书馆年份年报"格式
"""
        ...
        if last_year_links:
            ...
            if juesuan_links:
                ...
                for link_info in juesuan_links:
                    ...
                    # 修复URL：确保包含/dzfw/路径并使用HTTPS
                    if 'helib.net' in juesuan_url:
                        # 转换为HTTPS
                        if juesuan_url.startswith('http://'):
                            juesuan_url = juesuan_url.replace('http://', 'https://', 1)
                        
                        # 确保包含/dzfw/路径
                        if '/dzfw/' not in juesuan_url:
                            # 在域名后插入/dzfw/
                            if '://www.helib.net/' in juesuan_url:
                                juesuan_url = juesuan_url.replace('://www.helib.net/', '://www.helib.net/dzfw/', 1)
                            elif '://helib.net/' in juesuan_url:
                                juesuan_url = juesuan_url.replace('://helib.net/', '://helib.net/dzfw/', 1)
                            print(f"  修复URL为: {juesuan_url}")
...
def main():
    ...
    # 河北省图书馆年报页面URL
    report_page_url = "http://www.helib.net/dzfw/node_292.htm"
    base_url = "http://www.helib.net"
    ...
    filename = f"河北省图书馆{year_for_filename}年年报{file_ext}"
    filename = clean_filename(filename)
