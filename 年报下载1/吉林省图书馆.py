...
"""
吉林省图书馆年报下载脚本
功能：从吉林省图书馆官网下载去年的年报，并重命名为"吉林省图书馆年份年报"格式
"""
...
def find_last_year_report(url, base_url):
    ...
    try:
        ...
        # 直接查找标题对应的a标签中的链接
        all_links = []
        keywords = ['图书馆', '决算']  # 查找包含"图书馆"和"决算"的链接
        
        # 查找所有a标签，检查文本中是否同时包含"图书馆"和"决算"关键词
        for link in soup.find_all('a', href=True):
            ...
            # 检查链接文本中是否同时包含"图书馆"和"决算"关键词
            has_library = '图书馆' in text
            has_juesuan = '决算' in text
            
            # 必须同时包含"图书馆"和"决算"
            if not (has_library and has_juesuan):
                continue
            
            # 构建完整URL
            full_url = urljoin(url, href)  # 使用当前页面URL作为基础，而不是base_url
...
def main():
    ...
    # 吉林省图书馆年报页面URL
    report_page_url = "https://whhlyt.jl.gov.cn/zwgk/cwxx/js/202508/t20250825_9309519.html"
    base_url = "https://whhlyt.jl.gov.cn"
    ...
    filename = f"吉林省图书馆{year_for_filename}年年报{file_ext}"
    filename = clean_filename(filename)
